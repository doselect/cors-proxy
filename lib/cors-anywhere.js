...
simpleselect.func = function (node) {
  ...
  rs.on('data', function(data) {
    ...
    var linkRel = cheerio(str).attr('rel');
    var linkHref = cheerio(str).attr('href');
    if (linkRel === 'stylesheet'
        && linkHref
        && linkHref.substring(0, 4) !== 'http'
        && linkHref.substring(0, 2) !== '//') {
          console.log('HREF', linkHref, simpleselect.target.hostname);
      var href = `${simpleselect.target.protocol}//${simpleselect.target.hostname}/${linkHref}`;
      var newSS = `<link rel="stylesheet" href="${href}">`;
      styleSheetLinks += newSS;
    }
  });
...
function proxyRequest(req, res, proxy) {
  ...
  try {
    simpleselect.target = Object.assign({}, proxyOptions.target);
    harmon([], selects)(req, res, console.log);
...
exports.createServer = function createServer(options) {
  ...
  /* Next line is commented because
      of restrictd or no support for older browsers
      like firefox < 70 and safari
  */
  // app.use(helmet({ 
  //   frameguard: {
  //     action: 'allow-from',
